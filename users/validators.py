...
def validate_otp(value: str) -> None:
    ...
    if not value.isdigit() and not len(value) == 6:
        raise serializers.ValidationError(
            "OTP must be 6 digits long and contain only numbers."
        )


def validate_fullname(value: str) -> None:
    value = value.strip()

    # check if the full name contains only alphabets and spaces
    if not re.match("^[A-Za-z ]+$", value) and not len(value) > 2:
        raise serializers.ValidationError(
...
def validate_phone_not_registered(value: str) -> None:
...
def validate_phone_is_registered(value: str) -> None:
...
def validate_phone_has_agent_profile(value: str) -> None:
...
def validate_phone_no_agent_profile(value: str) -> None:
    user = User.objects.filter(phone=value).first()
